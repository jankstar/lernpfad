      ...
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="kurse">
          ...
          <div class="q-pa-md q-gutter-sm row">
            <q-select v-model="user_id" :options="getSubjects()" label="User" style="width: 100px"
              @popup-hide="onChangeUser" />
            <q-select v-model="kurs_id" :options="getKurse()" label="Kurs" style="width: 500px"
              @popup-hide="onChangeKurs" />

            <q-btn icon="published_with_changes" aria-label="Prüfen" @click="onCheck()" label="Prüfen"
              style="background: goldenrod; color: white">
              <q-tooltip>Änderungen prüfen</q-tooltip>
            </q-btn>
            <q-btn icon="restart_alt" aria-label="Zurücksetzen" @click="onReset()" label="Zurücksetzen"
              style="background: goldenrod; color: white">
              <q-tooltip>Änderungen zurücksetzen und Daten neu laden</q-tooltip>
            </q-btn>
            <q-btn icon="file_upload" aria-label="Sichern" @click="onSave()" label="Sichern"
              style="background: goldenrod; color: white">
        ...
        <q-tab-panel name="user">
          ...
          <q-btn @click="onRemoveGruppe()" label="Löschen"
            style="background: goldenrod; color: white; margin-top: 5px" />
          <div class="q-pa-md" v-if="user.admin && user.admin.allSPolicy && user.admin.allSPolicy.length > 0">
            <q-table :rows="user.admin.allSPolicy" row-key="id" :filter="filter" v-model:pagination="pagination1"
              :rows-per-page-options="[0]" selection="single" v-model:selected="selected1" style="height: 300px" />
          </div>
          ...
          <div class="q-pa-md" v-if="user.admin && user.admin.allGroups && user.admin.allGroups.length > 0">
            <q-table :rows="user.admin.allGroups" row-key="id" :filter="filter" v-model:pagination="pagination2"
              :rows-per-page-options="[0]" selection="single" v-model:selected="selected2" style="height: 300px" />
          </div>
        </q-tab-panel>
      </q-tab-panels>
    </div>
